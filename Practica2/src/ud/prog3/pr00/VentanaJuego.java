...
/**
 * Clase principal de minijuego de coche para Práctica 00 - Prog III Ventana del
 * minijuego.
 * 
...
//		});
	 ...
	 * 
	 * @param posX
	 *            Posición X de pixel del nuevo coche
	 * @param posY
	 *            Posición Y de píxel del nuevo coche
	 */
	...
	class MiRunnable implements Runnable {
		...
		public void run() {
			...
			while (sigo) {
				...
				miCoche.mueve(0.040);
				// Chequear choques
				if (miCoche.getPosX() < -JLabelCoche.TAMANYO_COCHE / 2
						|| miCoche.getPosX() > pPrincipal.getWidth() - JLabelCoche.TAMANYO_COCHE / 2) {
				...
				// Se comprueba tanto X como Y porque podría a la vez chocar en las dos
				// direcciones
				if (miCoche.getPosY() < -JLabelCoche.TAMANYO_COCHE / 2
						|| miCoche.getPosY() > pPrincipal.getHeight() - JLabelCoche.TAMANYO_COCHE / 2) {
